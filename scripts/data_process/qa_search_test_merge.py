...
if __name__ == '__main__':
    ...
    for data_source in data_sources:
        ...
        if 'test' in dataset:
            ...
            if data_source == 'hotpotqa':
                test_dataset = dataset['dev'].shuffle(seed=42).select(range(439*50))
            elif data_source == '2wikimultihopqa':
                test_dataset = dataset['dev'].shuffle(seed=42).select(range(73*50))
        else:
        ...
        print(f'Loaded {len(test_dataset)} examples from {data_source}')

    local_dir = args.local_dir
    hdfs_dir = args.hdfs_dir

    all_test_dataset = datasets.concatenate_datasets(all_dataset)
    all_test_dataset = all_test_dataset.map(
        lambda example, idx: {
            **example,
            "extra_info": {
                **example["extra_info"],
    ...
    all_test_dataset.to_parquet(os.path.join(local_dir, 'hotpotqa_2wiki_50steps.parquet'))
