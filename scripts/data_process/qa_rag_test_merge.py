...
if __name__ == '__main__':
    ...
    retrieval_caches = []
    files = os.listdir(args.retrieval_caches_dir)
    for data_source in data_sources:
        if f'{data_source}_test_top10.json' in files:
            retrieval_caches.append(args.retrieval_caches_dir+f'/{data_source}_test_top10.json')
        elif f'{data_source}_dev_top10.json' in files:
            retrieval_caches.append(args.retrieval_caches_dir+f'/{data_source}_dev_top10.json')
        else:
            raise ValueError(f'No retrieval cache found for {data_source}')
    all_dataset = []
    ...
    for index in range(len(data_sources)):
        ...
        if 'test' in dataset:
            print(f'Using the {data_sources[index]} test dataset...')
            test_dataset = dataset['test']
        elif 'dev' in dataset:
            print(f'Using the {data_sources[index]} dev dataset...')
            test_dataset = dataset['dev']
        else:
            raise ValueError(f'No test or dev dataset found for {data_sources[index]}')
