...
from rerank_server import CrossReranker, BiReranker
...
def get_reranker(config):
    if config.reranker_type == "sentence_transformer":
        ...
        elif 'Embedding' in config.rerank_model_name_or_path:
            return BiReranker.load(
                config.rerank_model_name_or_path,
                batch_size=config.batch_size,
            )
        else:
            raise ValueError(f"Unknown reranker model: {config.rerank_model_name_or_path}")
    else:
...
def search_endpoint(request: SearchRequest):
    ...
    # Step 2: Rerank
    reranked = reranker.rerank(request.queries, retrieved_docs, request.not_sort)
    # reranked = reranker.rerank(request.queries, retrieved_docs, True)
